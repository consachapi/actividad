              ...
              <div v-if="$route.meta.breadcrumb || $route.meta.pageTitle" class="router-header flex flex-wrap items-center mb-6">
                ...
                <vs-dropdown vs-trigger-click class="ml-auto md:block hidden cursor-pointer">
                  ...
                  <vs-dropdown-menu class="w-32">
                    <vs-dropdown-item>
                      <div @click="$router.push('/perfil').catch(() => {})" class="flex items-center">
...
export default {
  ...
  methods: {
    ...
    createMenu(){
      ...
      .then(response => {
        const role = response.data.data[0].roles.find(item => item === "llankasun_realm_admin");
        if(role === undefined){
          this.$vs.dialog({
              type: 'alert',
              color: 'danger',
              title: `Aviso`,
              text: 'Ud. no tiene acceso al sistema.',
              acceptText: 'Aceptar',
          });
        ...
        if(error.message === 'ERROR_NETWORK'){
          this.$vs.notify({
            title: 'Aviso',
            text: 'Ocurrio algún error en el sistema MiPROYECTO. Comunicarse con el Administrador',
            position:'top-center',
            iconPack: 'feather',
            icon: 'icon-alert-circle',
            color: 'danger'
